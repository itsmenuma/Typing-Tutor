...
void processAttempts(FILE* file) 
{
    ...
    while (1) 
    {
        char* currentPara = getRandomParagraph(file); //a random paragraph is stored in currentPara variable

        printf("\nType the following paragraph:\n%s\n", currentPara); //the random paragraph is displayed
        ...
        // Validate input: check for empty input or input with only whitespace
        ...
        double elapsedTime = (double)(endTime - startTime) / CLOCKS_PER_SEC; //elapsed time is calculated per second
